...
-- Quizzes table (must be created before questions that reference it)
...
CREATE TABLE IF NOT EXISTS questions (
  id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
  question_number INTEGER, -- Legacy field, kept for backward compatibility
  quiz_id UUID REFERENCES quizzes(id) ON DELETE CASCADE,
  quiz_question_number INTEGER,
...
CREATE TABLE IF NOT EXISTS quiz_sessions (
  ...
  created_at TIMESTAMP DEFAULT NOW(),
  updated_at TIMESTAMP DEFAULT NOW()
  -- Note: Removed UNIQUE(user_id) constraint to allow multiple quiz sessions
);
...
CREATE TABLE IF NOT EXISTS answers (
  ...
  UNIQUE(user_id, quiz_id, question_number)
);
